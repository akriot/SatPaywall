...
func main() {
    ...
    r := mux.NewRouter()
    r.HandleFunc("/api/getInvoice", getInvoiceHandler).Methods("GET")


	// unauthenticated
	api.HandleFunc("/claim", ClaimAddress).Methods("POST")
    api.HandleFunc("/getInvoice", getInvoiceHandler).Methods("GET")
    api.HandleFunc("/checkInvoice", checkInvoiceHandler).Methods("GET")
